"""Local data storage management under data/ folder.

Provides cache-aware loading: if a previous analysis exists on disk, it can
be returned directly without re-fetching from the network.
"""
...
def save_analysis_report(ticker: str, report_dict: dict) -> Path:
    ...
    # Maintain a latest copy for cache reads
...
def get_cache_age_hours(ticker: str) -> Optional[float]:
    ...
    try:
        ...
        if fetched:
            ...
            # Compare in local time (fetched_at is stored as naive local)
...
def list_saved_analyses(ticker: str) -> list[Path]:
    """List all saved analysis files for a ticker."""
    d = get_company_dir(ticker)
...
def drop_cache_ticker(ticker: str) -> bool:
    """Remove all cached data for a single ticker. Returns True if anything was removed."""
    d = get_data_root() / ticker.upper()
